...
We will be calling our application sol_currency. Open the terminal from your desired folder location and write down the following command:

```
npx create-react-app sol_currency
```
...
Let’s add a “connect to wallet” functionality to our application, that’ll allow your users to connect to sol_currency through their Phantom wallets.
...
The method to do that again is pretty straightforward, as we just have to set the minting authority of our tokens to _null_. Then no one would be able to mint more of our tokens and it can be seen publicly by viewing our token on https://explorer.solana.com/.
